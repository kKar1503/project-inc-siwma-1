...
import ParameterCreate from '../components/Modals/ParameterCreate';
...
const Page = () => {
  ...
  const refreshQuery = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ['getParameterCount'] });
  ...
  return (
    <div className="flex flex-col flex-1 w-full p-8 gap-8 overflow-auto">
      ...
      <ParameterCreate isOpen={isOpen} onRequestClose={closeModal} onSuccess={refreshQuery} />
      <NavBar />
      ...
      <div className="flex flex-row gap-6">
        <div className="mb-4 gap-4 flex-1">
          <div className="rounded-xl shadow-lg p-4 bg-base-100">
            <div className="pb-3">
              <h3 className="text-lg font-bold">Create a new parameter</h3>
              <p className="text-sm">Add a new parameter to the system</p>
            </div>
            <Link href="createParam/" className="btn w-full btn-outline btn-primary">
              Create Parameter
            </Link>
          </div>
        </div>
...
Page.getLayout = (page) => <AdminPageLayout pageName="Parameter">{page}</AdminPageLayout>;
